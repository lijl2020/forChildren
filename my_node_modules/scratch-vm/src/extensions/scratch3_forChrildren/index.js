...
class Scratch3ForChildrenBlocks {
     ...
     */
    step (args, util) {
        const steps = 25;//Cast.toNumber(args.STEPS);
    ...
    down(args, util){
        this._glide({SECS:1, X:util.target.x, Y:util.target.y-35}, util);
    }
    ...
    _onStepDone(util){
        ...
        if(GateConfig.checkIsArrivedPoint(util.target)){
            ...
            alert(`第${midCheckPointNum}个目的地打卡错误！`);
            util.stopAll();
        }
        //console.log(util);
    ...
    _glide (args, util) {
        if (util.stackFrame.timer) {
            ...
            if (timeElapsed < util.stackFrame.duration * 1000) {
                ...
                if (util.target.isTouchingObject("东方明珠-科技馆-世纪公园 关卡 合并 镂空")){
                    alert("任务失败，走到道路外面去了！");
                    util.stopAll();
                } else {
                    util.yield();
